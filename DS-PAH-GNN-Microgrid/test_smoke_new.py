...
def test_1_network_creation():
    ...
    for topo in ["mesh", "ring", "tree"]:
        ...
        print(f"{topo.upper()}: {meta['num_nodes']} nodes, {meta['num_edges']} edges")
...
def test_2_ev_pv_profiles():
    ...
    print(f"EV profiles: {len(ev_profiles)} stations")
    ...
    print(f"PV profiles: {len(pv_profiles)} feeders")
...
def test_5_constraint_checking(graph, results):
...
def test_6_time_series():
    ...
    print(f"Time-series: {len(time_series)} timesteps")
    for t, graph, meta in time_series[::8]:
...
def test_7_adversarial():
    ...
    print(f"Adversarial: {len(adversarial)} scenarios")
...
def test_8_results_export():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(f"Exported keys: {list(exported.keys())[:5]}")
